...
PostSchema.statics = {
  ...
  getListByTime: function (options, time) {
    return this.find({ ...options, created: { $lt: new Date(time) } })
      .sort({ created: -1 })
      .limit(10)
      .populate({
        path: "uid",
  ...
  findByTid: function (id) {
    return this.findOne({ _id: id }).populate({
      path: "uid",
      select: "name pic isVip _id unionid openid",
    });
  },
  ...
  getTopByDay: function (num, skip) {
    const querryOption = num
      ? { created: { $gte: moment().subtract(num, "days") } }
      : {};
    return this.find(querryOption, { answer: 1, title: 1, _id: 1 })
      .sort({ answer: -1 })
      .skip(skip * 15)
      .limit(15);
  },
